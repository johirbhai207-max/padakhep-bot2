...
def get_or_upload_files(folder_name):
    ...
    গুরুত্বপূর্ণ: upload ও chat-এ একই API key ব্যবহার নিশ্চিত করা হয়।
    """
    if folder_name in st.session_state.uploaded_files_cache:
        # ✅ Cache hit — upload-এ যে key ব্যবহার হয়েছিল সেটা আবার set করা
    ...
    # কোন key দিয়ে upload হচ্ছে সেটা save করা
        ...
        with st.chat_message("assistant"):
            ...
            while not success and attempts < len(VALID_KEYS):
                try:
                    ...
                    # Rate limit হলে key rotate ও সব cache reset
                    if any(x in error_msg.lower() for x in ["quota", "rate", "429", "limit"]):
                        mark_key_failed()
                        st.session_state.chat_session = None
                        st.session_state.uploaded_files_cache = {}
                        st.session_state.file_upload_key_index = None
